...
extension SPExtension where Base: UIImage{
  ...
  /// 图片尺寸: Bytes
  var sizeAsBytes: Int
  { get{ return base.jpegData(compressionQuality: 1)?.count ?? 0 } }
...
extension SPExtension where Base: UIImage{
  ...
  public func dealWithCrop(bound: CGRect) -> UIImage {
    ...
    let croppedImage: UIImage = UIImage(cgImage: imageRef!, scale: base.scale, orientation: UIImage.Orientation.up)
    return croppedImage
  }
