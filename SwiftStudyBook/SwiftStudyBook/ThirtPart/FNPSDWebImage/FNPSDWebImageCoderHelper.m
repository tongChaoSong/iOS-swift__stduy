...
@implementation FNPSDWebImageCoderHelper


+ (UIImage *)animatedImageWithFrames:(NSArray<FNPSDWebImageFrame *> *)frames {
    ...
    [frames enumerateObjectsUsingBlock:^(FNPSDWebImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
...
+ (NSArray<FNPSDWebImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            FNPSDWebImageFrame *frame = [FNPSDWebImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
    ...
    for (size_t i = 0; i < frameCount; i++) {
        @autoreleasepool {
            ...
            FNPSDWebImageFrame *frame = [FNPSDWebImageFrame frameWithImage:frameImage duration:frameDuration];
            [frames addObject:frame];
        }
    }
#endif
