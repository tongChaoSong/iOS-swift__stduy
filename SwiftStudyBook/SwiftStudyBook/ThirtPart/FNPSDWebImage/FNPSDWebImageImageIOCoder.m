...
@implementation FNPSDWebImageImageIOCoder {
...
+ (instancetype)sharedCoder {
    static FNPSDWebImageImageIOCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[FNPSDWebImageImageIOCoder alloc] init];
    });
    return coder;
}
...
- (UIImage *)decodedImageWithData:(NSData *)data {
    ...
    FNPSDImageFormat format = [NSData sd_imageFormatForImageData:data];
...
- (UIImage *)incrementallyDecodedImageWithData:(NSData *)data finished:(BOOL)finished {
...
#if SD_UIKIT || SD_WATCH
            _orientation = [FNPSDWebImageCoderHelper imageOrientationFromEXIFOrientation:orientationValue];
#endif
        }
    }
    ...
    if (_width + _height > 0) {
        ...
        if (partialImageRef) {
            ...
            CGColorSpaceRef colorSpace = FNPSDCGColorSpaceGetDeviceRGB();
...
- (UIImage *)decompressedImageWithImage:(UIImage *)image
    ...
    if (optionsDict != nil) {
        ...
        if ([optionsDict[FNPSDWebImageCoderScaleDownLargeImagesKey] isKindOfClass:[NSNumber class]]) {
            scaleDownLargeImagesOption = (NSNumber *)optionsDict[FNPSDWebImageCoderScaleDownLargeImagesKey];
        }
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(FNPSDImageFormat)format {
    ...
    if (format == FNPSDImageFormatUndefined) {
        BOOL hasAlpha = FNPSDCGImageRefContainsAlpha(image.CGImage);
        if (hasAlpha) {
            format = FNPSDImageFormatPNG;
        } else {
            format = FNPSDImageFormatJPEG;
        }
    }
...
#if SD_UIKIT || SD_WATCH
    NSInteger exifOrientation = [FNPSDWebImageCoderHelper exifOrientationFromImageOrientation:image.imageOrientation];
...
+ (BOOL)shouldDecodeImage:(nullable UIImage *)image {
    ...
    BOOL hasAlpha = FNPSDCGImageRefContainsAlpha(imageRef);
...
+ (BOOL)canEncodeToHEICFormat {
    ...
    dispatch_once(&onceToken, ^{
        NSMutableData *imageData = [NSMutableData data];
        CFStringRef imageUTType = [NSData sd_UTTypeFromSDImageFormat:FNPSDImageFormatHEIC];
    ...
    if (imageSource) {
        ...
        if (properties) {
            ...
            if (val) {
                ...
                result = [FNPSDWebImageCoderHelper imageOrientationFromEXIFOrientation:exifOrientation];
...
+ (CGColorSpaceRef)colorSpaceForImageRef:(CGImageRef)imageRef {
    ...
    if (unsupportedColorSpace) {
        colorspaceRef = FNPSDCGColorSpaceGetDeviceRGB();
    }
